...
BEGIN
    ...
    IF NOT (NEW.description_cs <=> OLD.description_cs) OR NOT (NEW.description_en <=> OLD.description_en) THEN
        ...
        IF @corpus_metadata_id IS NULL THEN
            ...
            INSERT INTO vlo_metadata_common (desc_cs, desc_en, corpus_metadata_id, contact_user_id)
                VALUES (COALESCE(NEW.description_cs,''), COALESCE(NEW.description_en,''), LAST_INSERT_ID(), @contact_user_id);
...
BEGIN
    IF NEW.type = 'corpus' AND (NOT (NEW.desc_cs <=> OLD.desc_cs) OR NOT (NEW.desc_en <=> OLD.desc_en)) THEN
        IF @skip_corpora_update IS NULL THEN
            ...
            UPDATE corpora SET description_cs = NULLIF(NEW.desc_cs,''), description_en = NULLIF(NEW.desc_en,'') WHERE name = (SELECT corpus_name FROM vlo_metadata_corpus WHERE id = NEW.corpus_metadata_id);
